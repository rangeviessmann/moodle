...
/**
 * Upgrade scripts for Preparation course format.
 *
...
/**
 * Upgrade script for Preparation course format.
 *
 * @param int|float $oldversion the version we are upgrading from
 * @return bool result
 */
function xmldb_format_preparation_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2023100901) {
        ...
        $sql = <<<EOF
                    UPDATE {course_sections}
                     ...
                     WHERE section > 0 AND (name IS NULL OR name = '')
                           AND course IN (SELECT id FROM {course} WHERE format = 'preparation')
        EOF;
        $DB->execute(
            sql: $sql,
        );
        ...
        upgrade_plugin_savepoint(true, 2023100901, 'format', 'preparation');
    }
