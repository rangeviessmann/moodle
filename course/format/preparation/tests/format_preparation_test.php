...
/**
 * Preparation course format related unit tests.
 *
 * @package    format_preparation
 * @copyright  2015 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers     \format_preparation
 */
final class format_preparation_test extends \advanced_testcase {

    /**
     * Tests for format_preparation::get_section_name method with default section names.
     *
     * @return void
     */
    public function test_get_section_name(): void {
        ...
        $numsections = 5;
        $course = $generator->create_course(['numsections' => $numsections, 'format' => 'preparation'],
            ['createsections' => true]);
    ...
    /**
     * Tests for format_preparation::get_section_name method with modified section names.
     *
     * @return void
     */
    ...
    /**
     * Tests for format_preparation::get_default_section_name.
     *
     * @return void
     */
    public function test_get_default_section_name(): void {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_preparation');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            } else {
                $sectionname = get_string('newsection', 'format_preparation');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            }
        }
    }
    ...
    public function test_update_inplace_editable(): void {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(['numsections' => 5, 'format' => 'preparation'],
            ['createsections' => true]);
        $section = $DB->get_record('course_sections', ['course' => $course->id, 'section' => 2]);

        // Call webservice without necessary permissions.
        try {
            \core_external::update_inplace_editable('format_preparation', 'sectionname', $section->id, 'New section name');
        ...
        $res = \core_external::update_inplace_editable('format_preparation', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable(): void {
        ...
        // Call callback format_preparation_inplace_editable() directly.
        $tmpl = component_callback('format_preparation', 'inplace_editable', ['sectionname', $section->id, 'Rename me again']);
    ...
    public function test_default_course_enddate(): void {
        ...
        $params = ['format' => 'preparation', 'numsections' => 5, 'startdate' => 1445644800];
    ...
    public function test_get_view_url(): void {
        ...
        $generator = $this->getDataGenerator();
        $course1 = $generator->create_course(['format' => 'preparation']);
    ...
    public function test_get_required_jsfiles(): void {
        ...
        $course = $generator->create_course(['format' => 'preparation']);
