...
/**
 * This file contains main class for Preparation course format.
 *
 * @since     Moodle 2.0
...
/**
 * Main class for the Preparation course format.
 *
 * @package    format_preparation
 * @copyright  2012 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_preparation extends core_courseformat\base {
    ...
    public function uses_indentation(): bool {
        return (get_config('format_preparation', 'indentation')) ? true : false;
    }
    ...
    /**
     * Returns the default section name for the preparation course format.
     *
    ...
    public function get_default_section_name($section) {
        ...
        if ($section->sectionnum == 0) {
            return get_string('section0name', 'format_preparation');
        }

        return get_string('newsection', 'format_preparation');
    }
     ...
     *
     * Preparation format uses the following options:
     * - coursedisplay
     * - hiddensections
     *
     ...
     *
     * In case if course format was changed to 'preparation', we try to copy options
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'preparation' allows to set and remove markers in addition to common section actions.
    ...
    public function get_config_for_external() {
        ...
        $formatoptions['indentation'] = get_config('format_preparation', 'indentation');
 ...
 */
function format_preparation_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            [$itemid, 'preparation'], MUST_EXIST);
