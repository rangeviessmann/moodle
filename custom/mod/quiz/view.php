...
/**
 * Quiz view page — custom override with training mode support.
 *
 * When a quiz has training mode enabled, this shows 3 buttons
 * (10, 20, 40 questions) instead of the standard start attempt button.
 *
 * @package   mod_quiz
...
// Helper: count actual question slots in a layout string.
$countlayoutslots = function(string $layout): int {
    $count = 0;
    foreach (explode(',', $layout) as $p) {
        if ((int) trim($p) > 0) {
            $count++;
        }
    }
...
// For training quizzes, clone $quiz per attempt with correct sumgrades
// so the standard renderer shows correct points (e.g. 5/10 not 5/62).
$istrainingquiz = !empty($quiz->training);

foreach ($attempts as $attempt) {
    if ($istrainingquiz && !empty($attempt->layout)) {
        $quizcopy = clone $quiz;
        $quizcopy->sumgrades = (float) $countlayoutslots($attempt->layout);
        $attemptobj = new quiz_attempt($attempt, $quizcopy, $cm, $course, false);
    } else {
...
if ($istrainingquiz && !empty($attempts)) {
    // For training: find best grade based on actual questions per attempt.
    $mygrade = null;
    $bestpercent = -1;
    foreach ($attempts as $att) {
        if ($att->state != quiz_attempt::FINISHED || is_null($att->sumgrades)) {
            continue;
        }
        $numslots = $countlayoutslots($att->layout);
        if ($numslots > 0) {
            $percent = $att->sumgrades / $numslots;
            if ($percent > $bestpercent) {
                $bestpercent = $percent;
                $mygrade = $percent * $quiz->grade;
            }
        }
    }
} else if (!$canpreview) {
...
// Training mode: if enabled, check if the question bank has questions
// (the quiz itself may have no slots — training draws random questions from the bank).
$showtrainingbuttons = false;
if (!empty($quiz->training) && !$unfinished && ($canattempt || $canpreview)) {
    // Count questions available in the question bank for this quiz context.
    $trainingquestioncount = $DB->count_records_sql(
        "SELECT COUNT(qbe.id)
    ...
    if ($trainingquestioncount > 0) {
        ...
        $viewobj->buttontext = ''; // Suppress normal start button.
        $viewobj->preventmessages = []; // Clear any prevent messages.
        // Tell the renderer the quiz has questions so it won't show "no questions" warning.
        $viewobj->quizhasquestions = true;
    }
}

$viewobj->showbacktocourse = ($viewobj->buttontext === '' && !$showtrainingbuttons &&
        course_get_format($course)->has_view_page());

echo $OUTPUT->header();

// Admin "Wyniki" button for preparation-course quizzes.
if (has_capability('mod/quiz:viewreports', $context) &&
        $DB->record_exists('local_recruitment_course', ['preparation_course' => $course->id])) {
    $reporturl = new moodle_url('/mod/quiz/report.php', ['id' => $cm->id, 'mode' => 'overview']);
    echo html_writer::div(
        $OUTPUT->single_button($reporturl, get_string('results', 'quiz'), 'get',
            ['class' => 'btn-secondary']),
        'mb-3'
    );
}
...
if (isguestuser()) {
...
} else if ($showtrainingbuttons) {
    // Training mode: render parts manually so buttons appear above results.
    echo $output->view_page_tertiary_nav($viewobj);
    echo $output->view_information($quiz, $cm, $context, $viewobj->infomessages);

    // Training buttons above results.
    $actionurl = new moodle_url('/mod/quiz/startattempt.php');
    echo '<div class="d-flex gap-3 justify-content-center my-4 flex-wrap">';
    foreach ([10, 20, 40] as $num) {
        if ($num > $trainingquestioncount) {
            continue;
        }
        $btnlabel = get_string('starttraining', 'quizaccess_training', $num);
        echo '<form method="post" action="' . $actionurl->out(false) . '">';
        echo '<input type="hidden" name="cmid" value="' . $cm->id . '">';
        echo '<input type="hidden" name="sesskey" value="' . sesskey() . '">';
        echo '<input type="hidden" name="numquestions" value="' . $num . '">';
    ...
    // Standard results and attempts list (with corrected sumgrades per attempt).
    echo $output->view_result_info($quiz, $context, $cm, $viewobj);
    echo $output->render($viewobj->attemptslist);
    echo $output->box($output->view_page_buttons($viewobj), 'quizattempt');
} else {
