...
namespace local_organizational\form;

defined('MOODLE_INTERNAL') || die();

require_once($GLOBALS['CFG']->libdir . '/formslib.php');

use local_organizational\organizational;

/**
 * Form for creating/editing an organizational matter.
 */
class organizational_form extends \moodleform {
    ...
    protected function definition() {
        ...
        // Name.
        $mform->addElement('text', 'name', get_string('organizationalname', 'local_organizational'), ['size' => 60]);
        ...
        $mform->addElement(
            'editor',
            'message_editor',
            get_string('organizationalcontent', 'local_organizational'),
            ['rows' => 15],
            organizational::editor_options($context)
        );
        $mform->setType('message_editor', PARAM_RAW);
        $mform->addRule('message_editor', get_string('required'), 'required', null, 'client');

        // Direction (required) — only show directions that don't have an organizational record yet.
        ...
        $sql = 'SELECT rc.id, ' . $DB->sql_concat('r.name', "' → '", 'rc.name') . ' AS fullname
                 ...
                 WHERE NOT EXISTS (
                       SELECT 1 FROM {local_organizational} o
                        WHERE o.directionid = rc.id' .
                       ($editid ? ' AND o.id <> :editid' : '') . '
                 )
        ...
        $mform->addElement('select', 'directionid', get_string('direction', 'local_organizational'), $options);
