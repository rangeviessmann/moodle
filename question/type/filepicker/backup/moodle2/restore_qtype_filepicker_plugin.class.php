 ...
 * needed to restore one filepicker qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_filepicker_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        return array(
            new restore_path_element('filepicker', $this->get_pathfor('/filepicker'))
        );
    }

    /**
     * Process the qtype/filepicker element
     */
    public function process_filepicker($data) {
        global $DB;
        ...
        // qtype_filepicker too.
        if ($questioncreated) {
            $data->questionid = $this->get_new_parentid('question');
            $newitemid = $DB->insert_record('qtype_filepicker_options', $data);
            $this->set_mapping('qtype_filepicker', $oldid, $newitemid);
        }
    }
    ...
    public static function define_decode_contents() {
        return array(
            new restore_decode_content('qtype_filepicker_options', 'graderinfo', 'qtype_filepicker'),
        );
    }

    /**
     * When restoring old data, that does not have the filepicker options information
     * in the XML, supply defaults.
     */
    protected function after_execute_question() {
        global $DB;

        $filepickerswithoutoptions = $DB->get_records_sql("
                    SELECT q.*
                      FROM {question} q
                      JOIN {backup_ids_temp} bi ON bi.newitemid = q.id
                 LEFT JOIN {qtype_filepicker_options} qeo ON qeo.questionid = q.id
                     WHERE q.qtype = ?
                       AND qeo.id IS NULL
                       AND bi.backupid = ?
                       AND bi.itemname = ?
                ", array('filepicker', $this->get_restoreid(), 'question_created'));

        foreach ($filepickerswithoutoptions as $q) {
            ...
            $DB->insert_record('qtype_filepicker_options', $defaultoptions);
        }
    }
}
