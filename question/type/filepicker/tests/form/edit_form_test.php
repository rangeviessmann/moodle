...
namespace qtype_filepicker\form;
...
require_once($CFG->dirroot . '/question/type/filepicker/edit_filepicker_form.php');

/**
 * Unit tests for the filepicker edit form.
 *
...
final class edit_form_test extends \advanced_testcase {
    ...
    public function test_attachments_validation(int $allowed, int $required, array $expected): void {
        list($form, $category) = $this->get_form('qtype_filepicker_edit_form');
        $submitteddata = [
    ...
    public static function user_preference_provider(): array {
        $valid = [];
        $invalid = ['attachmentsrequired' => get_string('mustrequirefewer', 'qtype_filepicker')];
        return [
