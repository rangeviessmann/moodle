 ...
 * @covers     \qtype_filepicker_question
 */
final class question_test extends \advanced_testcase {
    public function test_get_question_summary(): void {
        $filepicker = \test_question_maker::make_an_filepicker_question();
        $filepicker->questiontext = 'Hello <img src="http://example.com/globe.png" alt="world" />';
        $this->assertEquals('Hello [world]', $filepicker->get_question_summary());
    }
    ...
    public function test_summarise_response(int $responserequired, int $attachmentsrequired,
        ...
        // Create the filepicker question under test.
        $filepicker = \test_question_maker::make_an_filepicker_question();
        $filepicker->start_attempt(new question_attempt_step(), 1);

        $filepicker->responseformat = 'editor';
        $filepicker->responserequired = $responserequired;
        $filepicker->attachmentsrequired = $attachmentsrequired;
        ...
        $this->assertEquals($expected, $filepicker->summarise_response(
    ...
    public static function summarise_response_provider(): array {
        return [
            'text input required, not attachments required'  =>
                [1, 0, 'This is the text input for this filepicker.', 0, 'This is the text input for this filepicker.'],
            'Text input required, one attachments required, one uploaded'  =>
                [1, 1, 'This is the text input for this filepicker.', 1, 'This is the text input for this filepicker.Attachments: 0 (1 bytes)'],
    ...
    public function test_un_summarise_response(string $responseformat, string $summary, array $expected): void {
        ...
        $filepicker->responseformat = $responseformat;

        $this->assertEquals($expected, $filepicker->un_summarise_response($summary));
    }
    ...
    public function test_is_same_response(): void {
        ...
        $filepicker->responsetemplate = '';

        $filepicker->start_attempt(new question_attempt_step(), 1);

        $this->assertTrue($filepicker->is_same_response(
                array(),
                array('answer' => '')));
        ...
        $this->assertFalse($filepicker->is_same_response(
                array('answer' => 'Hello'),
                array()));
    ...
    public function test_is_same_response_with_template(): void {
        ...
        $filepicker->responsetemplate = 'Once upon a time';
    ...
    public function test_is_complete_response(): void {
        ...
        $filepicker->responserequired = 1;
        $filepicker->attachmentsrequired = 0;
        $filepicker->responseformat = 'editor';

        // The empty string should be considered an incomplete response, as should a lack of a response.
        $this->assertFalse($filepicker->is_complete_response(array('answer' => '')));
        $this->assertFalse($filepicker->is_complete_response(array()));

        // Any nonempty string should be considered a complete response.
        $this->assertTrue($filepicker->is_complete_response(array('answer' => 'A student response.')));
        $this->assertTrue($filepicker->is_complete_response(array('answer' => '0 times.')));
        $this->assertTrue($filepicker->is_complete_response(array('answer' => '0')));

        // Test case for minimum and/or maximum word limit.
        $response = [];
        $response['answer'] = 'In this filepicker, I will be testing a function called check_input_word_count().';

        $filepicker->minwordlimit = 50; // The answer is shorter than the required minimum word limit.
        $this->assertFalse($filepicker->is_complete_response($response));

        $filepicker->minwordlimit = 10; // The  word count  meets the required minimum word limit.
        $this->assertTrue($filepicker->is_complete_response($response));

        // The word count meets the required minimum  and maximum word limit.
        $filepicker->minwordlimit = 10;
        $filepicker->maxwordlimit = 15;
        $this->assertTrue($filepicker->is_complete_response($response));

        // Unset the minwordlimit/maxwordlimit variables to avoid the extra check in is_complete_response() for further tests.
        $filepicker->minwordlimit = null;
        $filepicker->maxwordlimit = null;

        // Test the case where two files are required.
        $filepicker->attachmentsrequired = 2;

        // Attaching less than two files should result in an incomplete response.
        $this->assertFalse($filepicker->is_complete_response(array('answer' => 'A')));
        $this->assertFalse($filepicker->is_complete_response(
        ...
        $this->assertTrue($filepicker->is_complete_response(
        ...
        $filepicker->responserequired = 0;
        $filepicker->responseformat = 'noinline';
        $filepicker->attachmentsrequired = 1;
    ...
    public function test_get_question_definition_for_external_rendering(): void {
        ...
        $filepicker = \test_question_maker::make_an_filepicker_question();
        $filepicker->minwordlimit = 15;
        $filepicker->start_attempt(new question_attempt_step(), 1);
        $qa = \test_question_maker::get_a_qa($filepicker);
        $displayoptions = new question_display_options();

        $options = $filepicker->get_question_definition_for_external_rendering($qa, $displayoptions);
        ...
        $this->assertEquals($filepicker->minwordlimit, $options['minwordlimit']);
    ...
    public function test_get_validation_error(int $responserequired,
        ...
        $question = \test_question_maker::make_an_filepicker_question();
    ...
    public static function get_min_max_wordlimit_test_cases(): array {
        return [
            ...
            'text input required, min word limit not reached'  => [1, 15, 25,
                get_string('minwordlimitboundary', 'qtype_filepicker', ['count' => 14, 'limit' => 15])],
            'text input required, max word limit is exceeded'  => [1, 5, 12,
                get_string('maxwordlimitboundary', 'qtype_filepicker', ['count' => 14, 'limit' => 12])],
    ...
    public static function get_word_count_message_for_review_test_cases(): array {
        return [
            ...
            'min and max, answer within range' =>
                    [10, 25, get_string('wordcount', 'qtype_filepicker', 14)],
            'min and max, answer too short' =>
                    [15, 25, get_string('wordcounttoofew', 'qtype_filepicker', ['count' => 14, 'limit' => 15])],
            'min and max, answer too long' =>
                    [5, 12, get_string('wordcounttoomuch', 'qtype_filepicker', ['count' => 14, 'limit' => 12])],
            'min only, answer within range' =>
                    [14, null, get_string('wordcount', 'qtype_filepicker', 14)],
            'min only, answer too short' =>
                    [15, null, get_string('wordcounttoofew', 'qtype_filepicker', ['count' => 14, 'limit' => 15])],
            'max only, answer within range' =>
                    [null, 14, get_string('wordcount', 'qtype_filepicker', 14)],
            'max only, answer too short' =>
                    [null, 13, get_string('wordcounttoomuch', 'qtype_filepicker', ['count' => 14, 'limit' => 13])],
        ];
    }
    ...
    private function create_user_and_sample_attachments($numberofattachments = 4) {
        ...
        $helper = \test_question_maker::get_test_helper('filepicker');
        $attachments = [];
